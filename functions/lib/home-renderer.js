  ...
  if (hasExternalEngines) {
    ...
    <div class="search-engine-btn-wrapper">
      <button class="search-engine-btn" onclick="event.stopPropagation();this.parentElement.querySelector('.search-engine-popup').classList.toggle('hidden')" aria-label="切换搜索引擎">
        ${defaultEngine.icon ? `<img src="${escapeHTML(defaultEngine.icon)}" class="search-engine-btn-icon" alt="">` : `<svg class="search-engine-btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>`}
